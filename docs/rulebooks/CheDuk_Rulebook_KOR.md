...
2.  **수반(首) 격파 루트** – 상대 국가수반을 포획 시 승리. 게임의 핵심은 대사관(使館)과 영역 시스템, 그리고 각 기물의 특수 능력입니다.
...
*   **이동:** 육각 방향으로 직선 1칸 또는 2칸 이동할 수 있습니다.
*   **수반 보호:** '국가수반(首)'의 6방향 인접 칸 중 하나에 경호원이 있을 경우, 수반이 공격받으면 경호원이 대신 제거됩니다.
*   **피격 규칙:** **수반 보호** 규칙에 의해 경호원이 제거될 경우, 수반 대신 **경호원만 제거**됩니다. 공격한 기물은 원래 위치에 남으며, 이는 공격 기물이 특사(特)일 경우에도 동일하게 적용됩니다.
...
1.  게임 보드에는 각 진영별로 중앙 기준점에서의 거리를 나타내는 숫자(이하 '배치 시작점')가 표시되어 있습니다. 양 플레이어는 상대가 모르는 상태에서, **자신의 색깔이면서 숫자 3 이하가 적힌 칸 중 하나**를 선택하여 '대사(使)'를 배치합니다. 이 위치가 '대사관'이 됩니다.
2.  중앙 기준점에서 더 가까운 거리에 대사를 배치한 플레이어가 선공을 잡습니다.

#### 3.3. 첩자 배치
1.  대사 배치가 끝나면, 후공 플레이어부터 번갈아 가며 **자신의 영역(Territory)이면서 자신의 색깔인 숫자가 표시된 배치 칸** 내에 '첩자(諜)' 5개를 배치합니다.
...
대사관 칸을 기준으로 특정 방향으로 그은 두 개의 선과 게임 판의 경계가 이루는 쐐기 모양의 공간이 해당 진영의 영역이 됩니다.
- **블루팀 영역**: 대사관에서 서쪽(W)과 북동쪽(NE)으로 그은 두 직선과 판의 경계로 둘러싸인 부분입니다.
- **레드팀 영역**: 대사관에서 동쪽(E)과 남서쪽(SW)으로 그은 두 직선과 판의 경계로 둘러싸인 부분입니다.
